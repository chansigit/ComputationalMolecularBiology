...
int main(int argc, char* argv[]){
    ...
    for (int i=0;i<seqCnt;++i){
        for (int j=i+1;j<seqCnt;++j){
            ...
            localAlignment(seqA,seqB,dp1,lAlignA,lAlignB,finalScoreL);
            cout<<"["<<idset[i]<<"]"<<endl<<"v.s."<<endl<<"["<<idset[j]<<"]"<<endl;
            cout<<"Alignment Score="<<finalScoreL<<"    Alignment Length="<<lAlignA.length()<<endl;
            printAlignment(lAlignA,lAlignB);
